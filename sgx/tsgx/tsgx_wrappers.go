...
/*
#cgo linux CFLAGS: -O3 -I../../tsgx-rs/target/release -I../../tsgx-rs/target/release/deps
#cgo linux LDFLAGS: -Wl,--no-as-needed -ldl -L../../tsgx-rs/target/release -l:libtsgx.a -L../../tsgx-rs/target/release/deps -l:libtsgx_c_api_dynamic_buffer.a -lm
#cgo darwin CFLAGS: -O3 -I../../tsgx-rs/target/release -I../../tsgx-rs/target/release/deps
#cgo darwin LDFLAGS: -framework Security -L../../tsgx-rs/target/release -ltsgx -L../../tsgx-rs/target/release/deps -ltsgx_c_api_dynamic_buffer -lm
...
func serialize(ptr unsafe.Pointer, t SgxUintType) ([]byte, error) {
	out := &C.DynamicBuffer{}
	var ret C.int
	switch t {
	case SgxBool:
		ret = C.serialize_sgx_bool(ptr, out)
	case SgxUint4:
		ret = C.serialize_sgx_uint4(ptr, out)
	case SgxUint8:
		ret = C.serialize_sgx_uint8(ptr, out)
	case SgxUint16:
		ret = C.serialize_sgx_uint16(ptr, out)
	case SgxUint32:
		ret = C.serialize_sgx_uint32(ptr, out)
	case SgxUint64:
		ret = C.serialize_sgx_uint64(ptr, out)
	case SgxUint160:
		ret = C.serialize_sgx_uint160(ptr, out)
	default:
...
func EncryptAndSerializeCompact(value uint64, sgxUintType SgxUintType) []byte {
	out := &C.DynamicBuffer{}
	switch sgxUintType {
	case SgxBool:
		val := false
		...
		C.public_key_encrypt_and_serialize_sgx_bool_list(pks, C.bool(val), out)
	case SgxUint4:
		C.public_key_encrypt_and_serialize_sgx_uint4_list(pks, C.uint8_t(value), out)
	case SgxUint8:
		C.public_key_encrypt_and_serialize_sgx_uint8_list(pks, C.uint8_t(value), out)
	case SgxUint16:
		C.public_key_encrypt_and_serialize_sgx_uint16_list(pks, C.uint16_t(value), out)
	case SgxUint32:
		C.public_key_encrypt_and_serialize_sgx_uint32_list(pks, C.uint32_t(value), out)
	case SgxUint64:
		C.public_key_encrypt_and_serialize_sgx_uint64_list(pks, C.uint64_t(value), out)
	case SgxUint160:
		// TODO
		...
		C.public_key_encrypt_and_serialize_sgx_uint160_list(pks, input, out)
	}
