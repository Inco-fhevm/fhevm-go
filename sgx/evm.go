...
import (
	...
	fhevm_crypto "github.com/zama-ai/fhevm-go/sgx/crypto"
	tsgx "github.com/zama-ai/fhevm-go/sgx/tsgx"
)
...
func importCiphertextToEVMAtDepth(environment EVMEnvironment, ct *tsgx.TsgxCiphertext, depth int) *verifiedCiphertext {
	existing, ok := environment.SgxData().verifiedCiphertexts[ct.GetHash()]
	if ok {
		...
		environment.SgxData().verifiedCiphertexts[ct.GetHash()] = new
		return new
	}
}

func importCiphertextToEVM(environment EVMEnvironment, ct *tsgx.TsgxCiphertext) *verifiedCiphertext {
	return importCiphertextToEVMAtDepth(environment, ct, environment.GetDepth())
}

func importCiphertext(environment EVMEnvironment, ct *tsgx.TsgxCiphertext) *verifiedCiphertext {
	return importCiphertextToEVM(environment, ct)
}

func importRandomCiphertext(environment EVMEnvironment, t tsgx.SgxUintType) []byte {
	nextCtHash := &environment.SgxData().nextCiphertextHashOnGasEst
	ctHashBytes := crypto.Keccak256(nextCtHash.Bytes())
	handle := common.BytesToHash(ctHashBytes)
	ct := new(tsgx.TsgxCiphertext)
	ct.SgxUintType = t
	ct.Hash = &handle
