...
func (m *ciphertextMetadata) deserialize(buf [32]byte) *ciphertextMetadata {
	...
	m.refCount = u[0]
	m.length = u[1]
	m.sgxUintType = tfhe.SgxUintType(u[2])
	return m
}
...
func persistIfVerifiedCiphertext(flagHandleLocation common.Hash, handle common.Hash, protectedStorage common.Address, env EVMEnvironment) {
	...
	if metadataInt.IsZero() {
		...
		metadata.refCount = 1
		metadata.length = uint64(tfhe.ExpandedSgxCiphertextSize[verifiedCiphertext.ciphertext.SgxUintType])
		metadata.sgxUintType = verifiedCiphertext.ciphertext.SgxUintType
